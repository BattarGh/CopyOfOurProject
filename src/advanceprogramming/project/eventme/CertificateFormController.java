...
public class CertificateFormController implements Initializable {
     ...
     * @param event معلومات اوامر المؤشر
     * @throws IOException لتفادي الغلط في حال ما وجد الملف المطلوب
     * @throws SQLException لتفادي الغلط في حال كان الامر المرسل الى قاعدة
     * البيانات غير صحيح
     */
    @FXML
    private void buttonOfOk(ActionEvent event) throws IOException, SQLException {

        rs = db.getStm().executeQuery("select users.fname,events.evdate_start,users.lname,events.evid from users INNER JOIN events "
                + "ON users.uid = events.evid where uid =  " + id);
        while (rs.next()) {
...
/**
 * 
 * @param name وضع الاسم 
 * @param email وضع الايميل
 * @param phone وضع رقم الهاتف
 * @param id  وضع رقم الهوية 
 */
